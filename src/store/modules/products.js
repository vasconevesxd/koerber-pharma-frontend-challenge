...
export default {
    ...
    mutations: {
        ...
        CURRENT_PAGE(state) {
            if (Array.isArray(state.productsFilter)) {
                let orderValues = state.productsFilter.slice((state.pages.currentPage - 1) * state.pages.pageSize, state.pages.currentPage * state.pages.pageSize)
    ...
    actions: {
        ...
        async fetchProductLimit({ commit }, gap) {
            return new Promise((resolve, reject) => {
                axios.get(`https://dummyjson.com/products?limit=5&skip=${gap}`)
                    .then(function (response) {
